...
import type { ChartPeriod, TransactionType, GroupingType } from '@/types/chart';

interface SimpleChartFormData {
  name: string;
  category_id: string;
  monthly_goal: string;
  color: string;
...
export default function AddChartModalSimple({ isOpen, onClose }: AddChartModalProps) {
  const { addChart, allCategories, allSubcategories } = useCharts();
  const { toast } = useToast();
  
  const [formData, setFormData] = useState<SimpleChartFormData>({
    name: '',
    category_id: '',
    monthly_goal: '',
